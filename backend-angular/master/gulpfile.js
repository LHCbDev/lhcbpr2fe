// ============================================================================
var gulp = require('gulp'), changed = require('gulp-changed'),
    concat = require('gulp-concat'), connect = require('gulp-connect'),
    expect = require('gulp-expect-file'), flip = require('css-flip'),
    fs = require('fs'), gulpFilter = require('gulp-filter'),
    gulpsync = require('gulp-sync')(gulp), gutil = require('gulp-util'),
    jade = require('gulp-jade'), less = require('gulp-less'),
    livereload = require(
    ...
    ngConstant = require('gulp-ng-constant'), path = require('path'),
...
var minifyOptions = {inliner: {timeout: minifyCSSTimeout}};
...
var vendor = {
  ...
  app: {source: require('./vendor.json'), dest: params.dist + '/vendor'}
};
// ============================================================================
// SOURCES CONFIG
var source = {
...
// BUILD TARGET CONFIG
var build = {
  scripts: {app: {main: 'app.js', dir: params.dist + '/app/js'}},
...
gulp.task('scripts:app', function() {
  ...
  ngConstant(
      {
        ...
        name: "buildParams",
        constants: {
          BUILD_PARAMS: {url_api: params.url_api, url_root: params.url_root}
        }
  ...
  return gulp.src(source.scripts.app)
      ...
      .on('end', function() {
        ...
        gutil.log('For any js errors see at ' + script +
                  '. For example:  head -n<error_line> ' + script);
      })
      .pipe(ngAnnotate())
...
gulp.task('styles:themes', function() {
  return gulp.src(source.styles.themes.main)
      .pipe(less({paths: [source.styles.themes.dir]}))
      .on("error", handleError)
      .pipe(gulp.dest(build.styles));
});
// ----------------------------------------------------------------------------
...
gulp.task('bootstrap', function() {
  return gulp.src(source.bootstrap.main)
      .pipe(less({paths: [source.bootstrap.dir]}))
      .on("error", handleError)
      .pipe(gulp.dest(build.styles));
});
// ----------------------------------------------------------------------------
...
gulp.task('connect', function() {
  connect.server({root: params.dist, port: params.port, livereload: false});
});
// ----------------------------------------------------------------------------
// MODULES SCRIPTS
// ----------------------------------------------------------------------------
gulp.task('modules:scripts', function() {
  ...
  var folders = fs.readdirSync(modulesPath).filter(function(file) {
...
gulp.task('modules:styles', function() {
  ...
  var tasks = folders.map(function(folder) {
    return gulp.src('modules/' + folder + '/less/style.less')
        .pipe(less({paths: ['modules/' + folder + '/less']}))
...
gulp.task('watch', function() {
  ...
  gulp.watch(source.styles.app.watch, watchOpts,
             ['styles:app', 'styles:app:rtl']);
...
// ----------------------------------------------------------------------------
// default (no minify)
gulp.task('default', gulpsync.sync(['start']), function() {

  gutil.log(gutil.colors.cyan('************'));
  gutil.log(
      gutil.colors.cyan('* All Done *'),
...
gulp.task('done', function() {
  console.log(
      'All Done!! You can start editing your code, LiveReload will update your browser after any change..');
});
// ----------------------------------------------------------------------------
...
// Error handler
function handleScriptError(err , script) {
  console.log(err.toString());
  this.emit('end');
}
