    ...
    scope.$watch("data", function(newValue, oldValue) {
      ...
      try {
        if (angular.isObject(newValue)) {
          ...
          scope.json = angular.fromJson(newValue);
        }
...
App.directive('rootjsserver', function($http) {
  ...
  link: function(scope) {
    var strFiles = _.map(_.keys(scope.files), encodeURIComponent).join(',')
    var url = scope.entrypoint + "/?files=" +
    strFiles +
    "&items=" + encodeURIComponent(scope.items) +
